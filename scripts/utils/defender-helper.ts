...
const getClient = async (): Promise<AdminClient> => {
  const apiKey: string = getEnvVar(process.env.DEFENDER_ADMIN_API_KEY);
  const apiSecret: string = getEnvVar(process.env.DEFENDER_ADMIN_API_SECRET);
...
export const proposeGrantRole = async (request: AccessControlRequest) => {
  const client = await getClient();
  const response = await client.proposeGrantRole(
    request.params,
    request.contract,
    request.role,
    request.account
...
export const proposeKYCAddressUpdate = async (
  ...
  const response = await proposeFunctionCall({
    contract: request.contract,
    params: request.params,
    functionName: request.functionName,
    functionInterface: [
...
export const proposeFunctionCall = async (
  ...
  const client = await getClient();
  return await client.createProposal({
    contract: request.contract,
