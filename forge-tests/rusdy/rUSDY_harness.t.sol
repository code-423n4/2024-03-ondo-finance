...
contract Test_rUSDY is USDY_BasicDeployment {
  ...
  function test_rUSDY_transfer_logs() public dealAndWrapAlice {
    vm.prank(alice);
    vm.expectEmit(true, true, true, true);
    emit Transfer(alice, bob, 1e18);
    vm.expectEmit(true, true, true, true);
    emit TransferShares(alice, bob, 1e22);
    rUSDYToken.transfer(bob, 1e18);
  }
  ...
  function test_rUSDY_wrap_logs() public {
    vm.prank(guardian);
    usdy.mint(alice, 500e18);

    vm.startPrank(alice);
    usdy.approve(address(rUSDYToken), 500e18);

    vm.expectEmit(true, true, true, true);
    emit Transfer(address(0), alice, 500e18);
    vm.expectEmit(true, true, true, true);
    emit TransferShares(address(0), alice, 500e22);

    rUSDYToken.wrap(500e18);
    vm.stopPrank();
  }
  ...
  function test_rUSDY_burn_logs() public dealAndWrapAlice {
    vm.prank(alice);
    vm.expectEmit(true, true, true, true);
    emit Transfer(address(rUSDYToken), alice, 50e18);
    /// @dev Add test after sharesBurnt() event rework
    rUSDYToken.unwrap(50e18);
  }
