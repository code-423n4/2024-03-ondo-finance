...
contract Test_DynamicOracle is USDY_BasicDeployment {
  ...
  function test_deploy_fail_invalidRange() public {
    vm.expectRevert(RWADynamicOracle.InvalidRange.selector);
    RWADynamicOracle oracle = new RWADynamicOracle(
      address(this),
      address(this),
      address(this),
      periodEnd + 1,
  ...
  function test_deploy_fail_invalidRangeStart() public {
    vm.expectRevert(RWADynamicOracle.InvalidRangeStart.selector);
    RWADynamicOracle oracle = new RWADynamicOracle(
      address(this),
      address(this),
      address(this),
  ...
  function test_dynamicOracle_setRange_invalidRate() public {
    vm.expectRevert(RWADynamicOracle.InvalidInterestRate.selector);
    oracleUSDY.setRange(periodEnd + 30 days, 1e26);
  }

  function test_dynamicOracle_setRange_invalidRangeTime() public {
    vm.expectRevert(RWADynamicOracle.InvalidRange.selector);
    oracleUSDY.setRange(periodEnd + 30.5 days, dailyIR);
  }
  ...
  function test_dynamicOracle_canSetNextRange_priorToCurrentClosing() public {
    oracleUSDY.setRange(periodEnd + 60 days, 1.00023368 * 1e27);
  ...
  function test_test_historical_view() public {
    setSeptRange();
    setOctRange();

    for (uint i = 0; i < prices_august.length; ++i) {
      uint256 price = oracleUSDY.getPriceHistorical(block.timestamp);
      assertEq(price, prices_august[i]);
      vm.warp(block.timestamp + 1 days);
    }
