...
abstract contract Test_RWAHub_Redemptions is
  BaseTestRunner,
  RWAHubEvents,
  ERC20Events
{
  ...
  function test_rwaHub_requestRedeem() public {
    ...
    vm.startPrank(alice);
    rwa.approve(address(rwaHub), 1000e18);

    vm.expectEmit(true, true, true, true);
    emit RedemptionRequested(alice, FIRST_REDEMPTION_ID, 1000e18);
    rwaHub.requestRedemption(1000e18);
    ...
    preRedeemProcessState(100_000e6)
  {
    ...
    redemptionIds.push(FIRST_REDEMPTION_ID);
    uint256 fiatRedeemed = 1000e6 * (pricer.getLatestPrice() / 1e18);
    vm.expectEmit(true, true, true, true);
