...
contract ASSERT_FORK_USDY_PROD is
  ...
  USDY_BasicDeployment
{
  ...
  function test_fork_assert_usdy_manager() public {
    ...
    assertEq(
      ...
      PROD_CONSTANTS_USDY_MAINNET.usdyhub_timestamp_setter_role
    );

    // // ASSERT USDY Manager config
    assertEq(
      usdyManager.assetSender(),
      PROD_CONSTANTS_USDY_MAINNET.asset_sender
    );
    assertEq(
      usdyManager.assetRecipient(),
      PROD_CONSTANTS_USDY_MAINNET.asset_recipient
    );
    assertEq(
      usdyManager.feeRecipient(),
      PROD_CONSTANTS_USDY_MAINNET.fee_recipient
    );
    assertEq(
      address(usdyManager.rwa()),
      PROD_CONSTANTS_USDY_MAINNET.usdy_asset
    );
    assertEq(
      address(usdyManager.collateral()),
      PROD_CONSTANTS_USDY_MAINNET.collateral
    );
    assertEq(
      address(usdyManager.pricer()),
      PROD_CONSTANTS_USDY_MAINNET.usdy_pricer
    );
    assertEq(
      usdyManager.minimumDepositAmount(),
      PROD_CONSTANTS_USDY_MAINNET.min_deposit_amt
    );
    assertEq(
      usdyManager.minimumRedemptionAmount(),
      PROD_CONSTANTS_USDY_MAINNET.min_redeem_amt
    );
    assertEq(usdyManager.mintFee(), PROD_CONSTANTS_USDY_MAINNET.mint_fee);
    assertEq(
      usdyManager.redemptionFee(),
      PROD_CONSTANTS_USDY_MAINNET.redeem_fee
    );
    assertEq(
      usdyManager.BPS_DENOMINATOR(),
      PROD_CONSTANTS_USDY_MAINNET.bps_denominator
    );
    assertEq(
      address(usdyManager.blocklist()),
      PROD_CONSTANTS_USDY_MAINNET.block_list
    );
    assertEq(
      address(usdyManager.sanctionsList()),
      PROD_CONSTANTS_USDY_MAINNET.sanctions_list
    );
    assertEq(
      usdyManager.decimalsMultiplier(),
      PROD_CONSTANTS_USDY_MAINNET.decimals_multiplier
    );
  }
  ...
  function test_fork_assert_usdy_token_proxy() public {
    ...
    assertEq(impl, PROD_CONSTANTS_USDY_MAINNET.usdy_impl_bytes);
    assertEq(admin, PROD_CONSTANTS_USDY_MAINNET.usdy_proxy_admin_bytes);

    // Assert that the owner of the proxy admin is correct
    assertEq(
      ProxyAdmin(address(uint160(uint256(admin)))).owner(),
      PROD_CONSTANTS_USDY_MAINNET.usdy_pa_owner
    );
    ...
    assertEq(
      ...
      PROD_CONSTANTS_USDY_MAINNET.usdy_minter_role
    );
    ...
    assertEq(usdy.paused(), PROD_CONSTANTS_USDY_MAINNET.paused);
    assertEq(usdy.decimals(), PROD_CONSTANTS_USDY_MAINNET.decimals);
    assertEq(usdy.name(), PROD_CONSTANTS_USDY_MAINNET.name);
    assertEq(usdy.symbol(), PROD_CONSTANTS_USDY_MAINNET.symbol);
  }
  ...
  function test_fork_assert_usdy_allowlist_proxy() public {
    ...
    assertEq(impl, PROD_CONSTANTS_USDY_MAINNET.allow_impl_bytes);
    assertEq(admin, PROD_CONSTANTS_USDY_MAINNET.allow_proxy_admin_bytes);

    // Assert that the owner of the proxy admin is correct
    assertEq(
      ProxyAdmin(address(uint160(uint256(admin)))).owner(),
      PROD_CONSTANTS_USDY_MAINNET.allow_pa_owner
    );
  ...
  function test_fork_assert_pricer() public {
    ...
    assertEq(
      ...
      PROD_CONSTANTS_USDY_MAINNET.pricer_default_admin
    );
    ...
    assertEq(
      ...
      PROD_CONSTANTS_USDY_MAINNET.pricer_price_update_role
    );
  }
}
