...
contract Test_SourceBridge is USDY_BasicDeployment, SourceBridgeEvents {
  ...
  function test_bridge_fail_cannot_bridge_zero() public initializeAlice {
    vm.expectRevert(SourceBridge.CannotBridgeZero.selector);
    vm.prank(alice);
    usdyBridge.burnAndCallAxelar{value: 0.01 ether}(0, "optimism");
  }
  ...
  function test_bridge() public initializeAlice {
    ...
    vm.expectEmit(true, true, true, true);
    emit BridgeInitiated(alice, 0, block.chainid, bytes32("1.0"), 100e18);
  ...
  function test_bridge_removeDestinationChainContractAddress_fails_nonOwner()
    public
  {
    vm.prank(alice);
    vm.expectRevert("Ownable: caller is not the owner");
    usdyBridge.removeDestinationChainContractAddress("optimism");
  }

  function test_bridge_fail_remove_chain_support() public initializeAlice {
    uint256 gasReceiverBalanceBefore = AXELAR_GAS_SERVICE.balance;
    ...
    assertEq(usdy.balanceOf(alice), 100e18);
    assertEq(AXELAR_GAS_SERVICE.balance, gasReceiverBalanceBefore);
  }
